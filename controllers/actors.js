...
var updateActor = (req, res) => {
  ...
  db.run(
    `UPDATE events SET actor_avatar_url = '${avatar_url}' WHERE actor_id = '${id}' AND  actor_login = '${login}';`,
    (err, rows) => {
      if (err) {
        return res.status(400).json({
          status: 400
...
var getStreak = (req, res) => {
  db.all(
    `select max(actor_id) as actor_id, actor_login, actor_avatar_url, count(*) as streak
